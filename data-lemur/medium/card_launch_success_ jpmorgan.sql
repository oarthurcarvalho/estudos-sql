...
-- Write a query that outputs the name of the credit card, and how many cards were issued in its launch month. The
-- launch month is the earliest record in the monthly_cards_issued table for a given card. Order the results
-- starting from the biggest issued amount.

WITH t1 AS (
  SELECT
    *,
    RANK() OVER (PARTITION BY card_name ORDER BY issue_year, issue_month ASC) AS launch_month
